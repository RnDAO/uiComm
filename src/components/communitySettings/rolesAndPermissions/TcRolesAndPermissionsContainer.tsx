...
function TcRolesAndPermissionsContainer() {
  ...
  const [adminByRole, setAdminByRole] = useState<IRoles[]>([{
    color: 0,
    deletedAt: null,
    id: "65d7e0a15f6147873b5799ff",
    name: "@everyone",
    roleId: "1012430565959553145"
  }]);
  const [adminByMember, setAdminByMember] = useState<IUser[]>([{
    avatar: "81b97383b6c75c643a07752bf7a35569",
    discordId: "338234552969330688",
    discriminator: "0",
    globalName: "George",
    ngu: "George",
    nickname: null,
  ...
  useEffect(() => {
    const fetchAndUpdateCommunity = async () => {
      ...
      const adminRoleIds = roles
        .filter((role: IPermissionsPayload) => role.roleType === 'admin' && role.source.identifierType === 'role')
        .map((filteredRole: IPermissionsPayload) => filteredRole.source.identifierValues)
        .flat();

      const adminMemberIds = roles
        .filter((role: IPermissionsPayload) => role.roleType === 'admin' && role.source.identifierType === 'member')
        .map((filteredRole: IPermissionsPayload) => filteredRole.source.identifierValues)
        .flat();

      const viewerRoleIds = roles
        .filter((role: IPermissionsPayload) => role.roleType === 'view' && role.source.identifierType === 'role')
        .map((filteredRole: IPermissionsPayload) => filteredRole.source.identifierValues)
        .flat();

      const viewerMemberIds = roles
        .filter((role: IPermissionsPayload) => role.roleType === 'view' && role.source.identifierType === 'member')
        .map((filteredRole: IPermissionsPayload) => filteredRole.source.identifierValues)
        .flat();
  ...
  const createPermissionPayload = (roleType: 'admin' | 'view', identifierType: 'member' | 'role', identifiers: string[]): IPermissionsPayload => ({
    roleType,
    source: {
      platform: "discord",
      identifierType,
  ...
  const handlePermissionsChange = () => {
    const adminMemberDiscordIds = adminByMember.map(member => member.discordId);
    const adminRoleRoleIds = adminByRole.map(role => role.roleId);
    const viewMemberDiscordIds = viewerByMember.map(member => member.discordId);
    const viewRoleRoleIds = viewerByRole.map(role => role.roleId);

    const permissionsPayload = [
      ...adminRoleRoleIds.length > 0 ? [createPermissionPayload('admin', 'role', adminRoleRoleIds)] : [],
      ...adminMemberDiscordIds.length > 0 ? [createPermissionPayload('admin', 'member', adminMemberDiscordIds)] : [],
      ...viewRoleRoleIds.length > 0 ? [createPermissionPayload('view', 'role', viewRoleRoleIds)] : [],
      ...viewMemberDiscordIds.length > 0 ? [createPermissionPayload('view', 'member', viewMemberDiscordIds)] : [],
    ];

    patchCommunityById({ communityId: community?.id as string, roles: permissionsPayload });
  };
  ...
  return (
    <div>
      <div className="w-full flex flex-col md:flex-row justify-between md:items-center space-y-3 md:space-y-0">
        ...
        <TcButton text="Save Changes" variant="contained" onClick={handlePermissionsChange} />
      </div>
      <div className='space-y-1.5 md:space-y-0'>
        <div className='py-1'>
