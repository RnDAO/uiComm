...
function TcNotionIntegration({ isLoading, connectedPlatforms, handleUpdateCommunityPlatoform }: TcNotionIntegrationProps) {
            ...
            {isLoading ? (
                ...
                connectedPlatforms.map((platform, index) => (
                    <Paper
                        ...
                        key={index}
                    >
                        <TcAvatar sizes='small' src={platform?.metadata?.owner?.user?.avatar_url}></TcAvatar>
                        <TcButton
                        ...
                        <div className='flex items-center space-x-3'>
                            {connectedPlatforms &&
                                <>
                                    <TcAvatar
                                        sizes='small'
                                        src={connectedPlatforms[0]?.metadata?.owner?.user?.avatar_url}
                                    />
                                    <div>
                                        <TcText
                                            text={connectedPlatforms[0]?.metadata?.owner?.user?.name + '(' + connectedPlatforms[0]?.metadata?.workspace_name + ')'}
                                            variant='h6'
                                            fontWeight='bold'
                                        />
                                        <TcText
