...
describe('TcHivemindDiscordLearnings', () => {
    ...
    it("should render learnings header", () => {
        render(<TcHivemindDiscordLearnings platform={mockPlatform} handleModuleConfigChange={() => jest.fn()} />);
        expect(screen.getByText('Learnings')).toBeInTheDocument();
    })
    it("should render data extraction placeholder", () => {
        render(<TcHivemindDiscordLearnings platform={mockPlatform} handleModuleConfigChange={() => jest.fn()} />);
        expect(screen.getByText('Select the data extraction period')).toBeInTheDocument();
    });
    it("should render Sync the following data sources", () => {
        render(<TcHivemindDiscordLearnings platform={mockPlatform} handleModuleConfigChange={() => jest.fn()} />);
        expect(screen.getByText('Sync the following data sources')).toBeInTheDocument();
    });
});